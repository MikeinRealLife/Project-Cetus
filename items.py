...
def use_item(item, player):
    if item == 'Apple':
        ...
        use_lockpick(player)
    elif item == 'Potion Of Cure Poison':
        use_potion_cure_poison(player)
    elif item == 'Espresso':
        use_espresso(player)
...
def use_stamina_potion(player):
    intelligence = player.totalIntelligence()
    missingStamina = player.totalStamina() + player.stamina
    frac = player.totalStamina()/5
    chance = randint(1,100) + intelligence

    if chance <= 30:
        player.stamina += min(frac, missingStamina)
    elif chance <= 60:
        player.stamina += min(frac*2, missingStamina)
    elif chance <= 85:
        player.stamina += min(frac*3, missingStamina)
    elif chance <= 95:
        player.stamina += min(frac*4, missingStamina)
    else:
...
def use_chicken_soup(player):
    if player.poisoned == True:
        player.poisoned = False
    remove_item('Chicken Soup', player)
...
def equip_amulet(item, player):
    ...
    if "Knowledge" in item:
        ...
        player.addedHealth = 3
    else:
        player.addedStamina = 2
