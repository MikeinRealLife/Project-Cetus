...
def splash_screen_func():
    ...
    for y, line in enumerate(splash_screen):
        screen.addstr(y+2,2, line)

    if os.path.exists('save.hero'):
        save = True
        screen.addstr(y+6, 2, "Would you like to load the previous game? (Y/N)")
        screen.move(y+7, 2)
        while choice != "Y" and choice != "N":
            screen.refresh()
            choice = screen.getkey()
            choice = choice.upper()
            screen.addstr(y+7, 2, choice)
        if choice == "Y":
            return False, None
        if choice == "N":
            os.remove('save.hero')
    ...
    if save == False or choice == "N":
        ...
        screen.addstr(y+6, 2, "Welcome To The Dungeon!")
        screen.addstr(y+8, 2, "Here Are The Fighters:")
        screen.addstr(y+9, 2, hero.Knight().hero_type + ": " + hero.Knight().desc())
        screen.addstr(y+10, 2, hero.Assassin().hero_type + ": " + hero.Assassin().desc())
        screen.addstr(y+11, 2, hero.Sorcerer().hero_type + ": " + hero.Sorcerer().desc())
        screen.addstr(y+13, 2, "Choose Your Character: (K/A/S)")
        screen.move(y+14, 2)
        while char != "K" and char != "A" and char != "S":
            screen.refresh()
            char = screen.getkey()
            char = char.upper()
            screen.addstr(y+14, 2, char)
        return True, char
