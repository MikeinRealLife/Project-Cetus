...
def open_menu(hero, pad, window, enemies, map_items):
    ...
    hero_status = curses.newwin(menuY, side_window_width, 3, menuX-side_window_width +2)
    hero_status.clear()
    hero_status.box()
    hero_status.border()


    menu = curses.newwin(menuY, menuX, 3, 3)
    menu.keypad(1)
    menu.clear()
...
def create_menu(hero, menu, cursor_pos):
    ...
    menu.addstr(inst_y-2, text_x_start, "Hit [x] to save and quit")
    menu.addstr(inst_y-3, text_x_start, "Hit [a] to unequip item")
    menu.addstr(inst_y-4, text_x_start, "Hit [d] to unequip weapon")
    menu.addstr(inst_y-5, text_x_start, "Hit [i] or [Esc] to return to game")
    menu.addstr(inst_y-6, text_x_start, "Hit [u] to use or equip selected item")
    ...
    if hero.items_inv:
        ...
        if selected in i.game_items.keys():
            menu.addstr(2,9, hero.items_inv.keys()[cursor_pos])
            menu.addstr(3, 5, i.game_items[selected][0])
        else:
            menu.addstr(2,9, hero.items_inv.keys()[cursor_pos])
            menu.addstr(3, 5, hero.items_inv[selected][1] + '. +' + str(hero.items_inv[selected][2]) + ' attack')
